...
<template>
  <div class="wrapper">
    <!-- header -->
    <div class="title">
      <em @click="back" class="back"></em>录制视频
    </div>
    <!-- content -->
    <div class="content">
        <div class="form-group">
          ...
          <div class="col-sm-4">
              <input type="button" :value="videoRecord.duration" >
          </div>
        </div><br>

        <input type="button" class="Sender" id="btn3" value='开始录制' @click="startVideo"><br>

        <div class="form-group">
          <label for="pass" class="control-label">文件地址：</label><br>
          <div class="col-sm-4">
            <select class="input-sm form-control short" id="get_cardtype2">
              <option :value="videoRecord.route">{{ videoRecord.route }}</option>
            </select>
          </div>
        </div><br>
...
module.exports = {
  ...
  methods: {
    ...
    startVideo: function() {
      var _this = this;
      try {
          cordova.plugins.AresMultimedia.videoRecord('record1', '0', 10, '0', success, error);
          function success(res){
              _this.videoRecord.route = res
          }
          function error(msg){
