<template>
  <div class="wrapper">
    <!-- header -->
    <div class="title">
      <em @click="back" class="back"></em>文档阅读
    </div>
    <!-- content -->
    <div class="content">
        <ul class="list">
            <li @click="onReading" v-for="(item,index) in readingList" :key="index"  class="technology">{{item.name}} <em class="arrow"></em></li>
        </ul>
    </div>
  </div>
</template>
...
module.exports = {
  ...
  methods: {
    ...
    onReading: function() {
      ...
      try {
        // ﬁlePath 文档路径
        cordova.plugins.AresDocument.pdfReader(ﬁlePath, '0', success, error);
